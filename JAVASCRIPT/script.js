...
// (Eliminado: window._isContinuingGame = false;)
...
function mostrarInicio1() {
  ...
  // --- AÑADIDO: Ocultar botón de guardar al volver al menú ---
  const btnSave = document.getElementById("btnGuardarPartida");
...
// ==================================================================
// ----------               continuarPartida                ----------
//            (REESCRITA v3, con carga de guardado)
// ==================================================================
async function continuarPartida() {
  ...
  let saveData = null;
  try {
    // 1. Buscar el guardado del usuario
    console.log("Buscando partida guardada...");
    const res = await fetch('../PHP/get_save.php', { cache: 'no-store' });
    if (res.ok) {
        const data = await res.json();
        if (data.success && data.save) {
            saveData = data.save;
            console.log("Partida encontrada:", saveData);
        } else {
            console.warn("No se encontró partida guardada en el servidor.");
        }
    }
  } catch (e) {
    console.error("Error al buscar partida guardada:", e);
  }

  // 2. Iniciar el juego.
  // iniciarJuego se encargará de usar 'scene_1' si saveData es null,
  // o usará saveData.scene_slug si existe.
  await iniciarJuego(saveData);
}
...
async function nuevaPartida() {
  ...
  // 3. Crear/Reiniciar el guardado en el servidor (BORRA PROGRESO ANTERIOR)
  try {
    // Este es el estado por defecto de un juego nuevo
    const defaultGameState = {
        scene_slug: "scene_1",
        saved_at: (new Date()).toISOString(),
        gameState: {
            customInteractables: null, // Usará los de la escena por defecto
            disabledInteractables: [], // Ninguno deshabilitado
            completedScenes: {}      // Ninguna escena completada
        }
    };
    
    const nuevaPartidaPayload = {
      scene_id: 1, // ID de la escena 1
      scene_slug: "scene_1", 
      state_json: defaultGameState // Guardamos el estado "limpio"
    };
    ...
    if (data && data.success) {
      // El servidor guardó el reseteo, ahora actualizamos localmente
    ...
    // Fallback si falla el guardado: al menos se marca como iniciada localmente
    partidaEstado = "iniciada";
    localStorage.setItem("partida_estado", "iniciada");
    actualizarBotonesSegunEstado();
  }
...
async function iniciarJuego(saveData = null) {
  ...
  // --- AÑADIDO: Mostrar botón de guardar ---
  const btnSave = document.getElementById("btnGuardarPartida");
  ...
  try {
        ...
        // Si pasamos datos de guardado (desde "Continuar"), los usamos
        if (saveData && saveData.scene_slug) {
            startSlug = saveData.scene_slug;
            console.log('Datos de guardado encontrados, iniciando en:', startSlug);
        } else {
            console.log('No hay datos de guardado, iniciando en scene_1');
        }
    ...
    // Después de cargar la escena, aplicamos el estado guardado.
    if (saveData && saveData.state_json && saveData.state_json.gameState) {
        console.log("Restaurando estado del juego...", saveData.state_json.gameState);
        const gameState = saveData.state_json.gameState;
        
        // Si hay interactuables guardados, los cargamos. Si no, dejamos los que la escena cargó.
        if (gameState.customInteractables) {
           window.customInteractables = gameState.customInteractables;
        }
        // Restauramos la lista de interactuables deshabilitados
        window.DISABLED_INTERACTABLES = new Set(gameState.disabledInteractables || []);
        
        // Restauramos el historial de escenas 'slides' completadas
        window.COMPLETED_SCENES = gameState.completedScenes || {};
...
function ensureFadeOverlay() {
  ...
  if (!overlay) {
    ...
    Object.assign(overlay.style, {
      ...
      zIndex: '200000', // MODIFICADO: z-index alto (ajuste)
      pointerEvents: 'none'
    });
