...
async function continuarPartida() {
  ...
  // Simple fallback: marcar continuación e iniciar el juego directamente.
...
async function iniciarJuego() {
  ...
  try {
        ...
        if (window._isContinuingGame) {
            try {
                // Hacemos el fetch al script que ya sabemos que funciona
                const res = await fetch('../PHP/get_save.php', { cache: 'no-store' });
                const data = await res.json();
                
                // ¡CORRECCIÓN CLAVE AQUÍ!
                // Si data.success es true Y data.save existe Y tiene scene_slug:
                if (data.success && data.save && data.save.scene_slug) {
                    startSlug = data.save.scene_slug;
                    console.log('get_save success, iniciando en:', startSlug);
                } else {
                    console.warn('get_save exitoso, pero no se encontró scene_slug. Iniciando en scene_1');
                }
            } catch (e) {
                console.warn('get_save falló (fetch error), iniciando en scene_1', e);
            }
