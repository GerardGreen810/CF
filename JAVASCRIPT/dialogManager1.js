/* dialogManager.js
   ...
   - Exponer:
     ...
     dialogManager.startDialogForTrigger(signal)  // consulta al servidor si quieres
...
async function load(url = '../data/dialogs.json') {
  ...
  function startTyping(textEl, fullText) {
    ...
    // trim opcional: fullText = fullText.replace(/\r\n/g, '\n');
    textEl.textContent = '';
    ...
    // si texto vacío, terminamos inmediatamente (genera render final)
...
// conectar tecla E: si dialog activo, avanzar; si no, ignorar
window.addEventListener('keydown', (ev) => {
  if (!current) return;
  // ignorar si el diálogo se abrió ahora mismo (evita que la misma pulsación que abrió
  // lo haga avanzar)
  if (current._justOpened) {
    console.debug('[dialogManager] keydown ignorada por justOpened');
    return;
  }
